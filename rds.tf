...
resource "aws_db_instance" "postgres" {
  ...
  allocated_storage               = "${var.environment == "prod" ? "3300" : "450"}"
  storage_type                    = "gp2"
  engine                          = "postgres"
...
resource "aws_db_instance" "internal_reporting_snapshot_stg" {
  ...
  snapshot_identifier                 = "${var.environment == "prod" ? "rds:postgres-internal-tableau-apps-prod-dq-2020-05-21-00-07" : "rds:postgres-internal-tableau-apps-notprod-dq-2020-03-23-07-07"}"
  auto_minor_version_upgrade          = "true"
  backup_retention_period             = "14"
  copy_tags_to_snapshot               = "false"
  db_subnet_group_name                = "${aws_db_subnet_group.rds.id}"
