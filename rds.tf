...
resource "aws_db_instance" "internal_reporting_snapshot_stg" {
  ...
  enabled_cloudwatch_logs_exports     = ["postgresql", "upgrade"]
  iam_database_authentication_enabled = "false"
  identifier                          = "staging-postgres-${local.naming_suffix}"
...
resource "aws_db_instance" "internal_reporting_snapshot_staging" {
  count                               = "${local.internal_reporting_stg_count}"
  snapshot_identifier                 = "rds:postgres-internal-tableau-apps-prod-dq-2019-07-03-00-09"
  auto_minor_version_upgrade          = "true"
  backup_retention_period             = "14"
  backup_window                       = "00:00-01:00"
  copy_tags_to_snapshot               = "false"
  ...
  tags {
    Name = "staging-postgres-${local.naming_suffix}"
  }
}
