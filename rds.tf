...
resource "aws_db_instance" "internal_reporting_snapshot_stg" {
  ...
  snapshot_identifier                 = var.environment == "prod" ? "rds:postgres-internal-tableau-apps-prod-dq-2022-06-09-00-09" : "rds:postgres-internal-tableau-apps-notprod-dq-2022-05-05-07-08"
  auto_minor_version_upgrade          = "false"
  backup_retention_period             = "14"
  copy_tags_to_snapshot               = "false"
  db_subnet_group_name                = aws_db_subnet_group.rds.id
  ...
  performance_insights_enabled          = true
  performance_insights_retention_period = "7"
  parameter_group_name                  = var.environment == "prod" ? "postgres10-mem" : "dqpostgres10"
...
resource "aws_db_instance" "internal_reporting_snapshot_stg2" {
  count                               = local.internal_reporting_stg2_count
  snapshot_identifier                 = var.environment == "prod" ? "rds:postgres-internal-tableau-apps-prod-dq-2022-04-28-00-09" : "rds:postgres-internal-tableau-apps-notprod-dq-2022-05-05-07-08"
  auto_minor_version_upgrade          = "false"
  backup_retention_period             = "14"
  copy_tags_to_snapshot               = "false"
  db_subnet_group_name                = aws_db_subnet_group.rds.id
  ...
  enabled_cloudwatch_logs_exports     = ["postgresql", "upgrade"]
  iam_database_authentication_enabled = "false"
  identifier                          = "stg2-postgres-${local.naming_suffix}"
  instance_class                      = var.environment == "prod" ? "db.m5.4xlarge" : "db.m5.2xlarge"
  iops                                = "0"
  kms_key_id                          = data.aws_kms_key.rds_kms_key.arn
  license_model                       = "postgresql-license"
  ...
  performance_insights_enabled          = true
  performance_insights_retention_period = "7"
  parameter_group_name                  = "akertest-mergejoin-off"
  ...
  tags = {
    Name = "stg2-postgres-${local.naming_suffix}"
  }
}
